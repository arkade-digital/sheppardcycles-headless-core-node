...
exports.wrapper = promise => promise.then(result => {
    if (result.errors) {
        throw result.errors;
    }
    return result;
...
exports.traverseParents = node => {
    let handles = [];
    if (node.parent)
        handles.push(...exports.traverseParents(node.parent));
...
exports.writeRobots = (data) => __awaiter(this, void 0, void 0, function* () {
    if (data.length > -1) {
        try {
...
exports.writeProducts = (config, data, template, createPage) => {
    ...
    data.forEach(({ node }) => {
        var _a, _b, _c, _d, _e;
        createPage({
            path: `${(_a = config.settings) === null || _a === void 0 ? void 0 : _a.routes.PRODUCT}/${node.handle}`,
            component: template,
            context: {
                handle: node.handle,
...
exports.writeCollections = (config, data, template, createPage) => {
    ...
    data.forEach(({ node }) => {
        var _a;
        createPage({
            path: `${(_a = config.settings) === null || _a === void 0 ? void 0 : _a.routes.COLLECTION}/${exports.nestedHandles(node)}`,
            component: template,
            context: {
...
exports.writeDocument = (url, data, template, createPage) => data.forEach(({ node }) => {
    var _a, _b;
    createPage({
        path: `${url}/${((_a = node === null || node === void 0 ? void 0 : node.handle) === null || _a === void 0 ? void 0 : _a.current) || ((_b = node === null || node === void 0 ? void 0 : node.slug) === null || _b === void 0 ? void 0 : _b.current)}`,
        component: template,
        context: {
            id: node._id,
        },
...
exports.proxyMiddleware = (config, from, to) => http_proxy_middleware_1.createProxyMiddleware({
    target: "http://localhost:5001",
    pathRewrite: {
        [`${from}`]: `/${config.services.firebase.projectId}/${config.services.firebase.region}/${to}`,
    },
});
exports.proxyUrl = (config, to) => `http://localhost:5001/${config.services.firebase.projectId}/${config.services.firebase.region}/${to}`;
...
exports.buildSanityConfig = config => (Object.assign({ projectId: config.services.sanity.projectId, dataset: config.stores[config.services.shopify.defaultShopName].sanityDataset }, (exports.isDevelopment && { token: config.services.sanity.token, overlayDrafts: true, watchMode: true })));
